...
def calculator():
    ...
    while True:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            for entry in listdb:
                                ...
                                counter = counter + 1
                            print(cleanlist)
                            table_check = input('Input Table to rename: ')
                            if table_check == 'back':
                                break
                            if table_check == 'exit':
                                sys.exit()
                            if table_check in cleanlist:
                                newcmd = "SELECT ID, Product, SUM(Machines) AS Machines, SUM(Wattage) AS Wattage,SUM(Output) AS Output FROM " + table_check+ " GROUP BY ID, Product HAVING COUNT(*) = 1;"
                                newcmd2 = "SELECT ID, Product, SUM(Machines) AS Machines, SUM(Wattage) AS Wattage,SUM(Output) AS Output FROM " + table_check + " GROUP BY ID, Product HAVING COUNT(*) > 1;"
                                neatdict = dict()
                                neatdict['key'] = None
                                neatdict['ID'] = None
                                ...
                                while True:
                                    table = input('Input New Table Name: ')
                                    try:
                                ...
                                for row in cursor.fetchall():
                                    ...
                                    cursor.execute(insertcmd, values)
                                    cnxn.commit()
                                cursor.execute('DROP TABLE '+table_check+';')
                        except IndexError:
                            print('Database is Empty.')
                            break
                if usercmd == 'view':
                        ...
                        try:
                            ...
                            if table_check in cleanlist:
                                ...
                                for row in cursor.fetchall():
                                    ...
                                    if getcheck['ID'] not in ('Raw', 'BYPRODUCT'):
                                        print(getdict)
                                    if getcheck['ID'] == 'BYPRODUCT':
                                        ...
                                        del getdict['ID']
                                cnxn.commit()
                                getcmd = "SELECT ID, SUM(Machines) AS Machines FROM "+table_check+" GROUP BY ID;"
                                cursor.execute(getcmd)
                                for row in cursor.fetchall():
                                    machines['Machine'] = row[0]
                                    machines['Count'] = row[1]
                                    if machines['Machine'] != 'Raw':
                                        print(machines)
                                cnxn.commit()
                                getcmd = "SELECT ID, Product, SUM(Output) AS Output FROM "+table_check+" GROUP BY ID, Product;"
                                cursor.execute(getcmd)
                                for row in cursor.fetchall():
                                    ore_check['ID'] = row[0]
                                    ore['Ore'] = row[1]
                                    ...
                                    if ore_check['ID'] == 'Raw':
                                        print(ore)
                                getcmd = "SELECT SUM(Wattage) AS Wattage FROM "+table_check+";"
                                cursor.execute(getcmd)
                                for row in cursor.fetchall():
                                    watts['Total Wattage'] = row[0]
                                    print(watts)
                                cnxn.commit()
                            if table_check not in cleanlist:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            for entry in listdb:
                                ...
                                counter = counter + 1
                            print(cleanlist)
                            modtable = input('Enter Table to Modify: ')
                            if modtable == 'exit':
                                sys.exit()
                            if modtable == 'back':
                                break
                            if modtable == 'list':
                                for file in glob.glob('*.json'):  # Grabbing File
                                    with open(os.path.join(os.getcwd(), file), 'r') as f:
                                        print(file.replace('.json', ''))
                            if modtable in cleanlist:
                                try:
                                    moddb_var = "SELECT * FROM "+modtable+" WHERE `key` = 1;"
        ...
        while True:                             #CREATING RECIPE ENTRY IN SQL DB
            if modding == 1:
                drop_table = "DROP TABLE IF EXISTS "+modtable+";"
                cursor.execute(drop_table)
                cnxn.commit()
                create_table = 'CREATE TABLE ' + modtable + "dirty (`key` INT NOT NULL AUTO_INCREMENT, `ID` VARCHAR(255) NULL, `Product` VARCHAR(255) NULL, `Output` FLOAT NULL, `Machines` FLOAT NULL, `Wattage` FLOAT NULL, PRIMARY KEY (`key`));"
                cursor.execute(create_table)
                cnxn.commit()
                table = modtable
                break
        ...
        while True:
            ...
            if 'Raw' not in d:  #BASE LAYER
                ...
                getcmd = "SELECT ID, Product, SUM(Machines) AS Machines, SUM(Wattage) AS Wattage,SUM(Output) AS Output FROM "+table+" GROUP BY ID, Product HAVING COUNT(*) = 1;"
                getdict = dict()
                getcheck = dict()
                watts = dict()
                machines = dict()
                ...
                for row in cursor.fetchall():
                    ...
                    if getcheck['ID'] == 'BYPRODUCT':
                        ...
                        del getdict['ID']
                cnxn.commit()
                getcmd = "SELECT ID, SUM(Machines) AS Machines FROM "+table+" GROUP BY ID;"
                cursor.execute(getcmd)
                for row in cursor.fetchall():
                    machines['Machine'] = row[0]
                    machines['Count'] = row[1]
                    if machines['Machine'] not in ('Raw', 'BYPRODUCT'):
                        print(machines)
                cnxn.commit()
                getcmd = "SELECT ID, Product, SUM(Output) AS Output FROM "+table+" GROUP BY ID, Product;"
                cursor.execute(getcmd)
                for row in cursor.fetchall():
                    ore_check['ID'] = row[0]
                    ore['Ore'] = row[1]
                    ...
                    if ore_check['ID'] == 'Raw':
                        print(ore)
                cnxn.commit()
                getcmd= "SELECT SUM(Wattage) AS Wattage FROM "+table+";"
                cursor.execute(getcmd)
                for row in cursor.fetchall():
                    watts['Total Wattage'] = row[0]
                    print(watts)
                ...
                while True:
                    ...
                    if satisfied.lower() in ('yes', 'y'):
                        break
                    if satisfied.lower() in ('no', 'n'):
                        while True:
                            try:
                                out = int(input('Enter New Output: '))
                                break
                            except ValueError:
                                print('Please Enter a Whole Number.')
                        break
                    if satisfied.lower() not in ('y','n','yes','no'):
                        print('(Y OR N)')
                if satisfied.lower() in ('yes', 'y'):
                    break